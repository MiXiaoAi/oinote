        ...
        <div v-if="isCollabEnabled" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-base-200">
          ...
          <div v-if="isCollabConnected && isCollabSynced" class="flex -space-x-2 sm:ml-1">
            ...
            <div class="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold border-2 border-base-100 bg-primary"
                 :title="`${authStore.user?.nickname || authStore.user?.username || '我'} (你)`">
              {{ (authStore.user?.nickname || authStore.user?.username || '我').charAt(0).toUpperCase() }}
            </div>
            <!-- 显示其他用户 -->
            <div v-for="[clientId, cursor] in Array.from(remoteCursors.entries()).slice(0, 3)" 
                 ...
                 :title="cursor.nickname || cursor.username">
              {{ (cursor.nickname || cursor.username).charAt(0).toUpperCase() }}
            </div>
...
import { RemoteCursorExtension } from '../utils/remote-cursor';
...
const initCollab = () => {
  ...
  try {
    collabClient.value = new YjsCollabClient(
      ...
      authStore.user.id,
      authStore.user.username || `User${authStore.user.id}`,
      authStore.user.nickname || authStore.user.username || `User${authStore.user.id}`
    );
    ...
    collabClient.value.onUserLeft = (user) => {
      ...
      updateRemoteCursors(newCursors);
    };
...
const syncYjsToEditor = (content) => {
  ...
  try {
    ...
    // 更新远程光标显示
    nextTick(() => {
      updateRemoteCursors(remoteCursors.value);
    });
  } finally {
...
const handleRemoteCursor = (awareness) => {
  if (!awareness.cursor) {
    ...
    remoteCursors.value.set(awareness.clientId, {
      ...
      nickname: awareness.nickname || awareness.username,
...
// 更新远程光标到扩展
const updateRemoteCursors = (cursors) => {
  if (!editor) return;
  
  // 查找 RemoteCursorExtension 并更新其选项
  const extension = editor.extensionManager.extensions.find(
    ext => ext.name === 'remoteCursor'
  );
  
  if (extension) {
    extension.options.cursors = cursors;
    // 触发 ProseMirror 状态更新以重新渲染装饰器
    // 使用 appendTransaction 来触发插件重新计算装饰器
    const { state, dispatch } = editor.view;
    dispatch(state.tr.setMeta('remoteCursorUpdate', true));
  }
};
...
.remote-cursor-widget:hover .remote-cursor-label,
.remote-cursor-caret:hover + .remote-cursor-label {
  opacity: 1;
  transform: translateY(0);
}
