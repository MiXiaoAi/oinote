...
export class YjsCollabClient {
  constructor(noteId, userId, username, nickname, wsUrl) {
    this.noteId = noteId
    this.userId = userId
  ...
  /**
   * 获取用户颜色（基于 userId 的确定性分配）
   */
  getUserColor(userId) {
    if (!this.userColors.has(userId)) {
      // 使用 userId 的哈希值来确定颜色，确保同一用户在所有客户端上获得相同颜色
      const hash = typeof userId === 'number' ? userId : String(userId).split('').reduce((a, b) => a + b.charCodeAt(0), 0)
   ...
   */
  connect() {
    const url = `${this.wsUrl}?userId=${this.userId}&username=${encodeURIComponent(this.username)}&nickname=${encodeURIComponent(this.nickname)}&noteId=${this.noteId}`
