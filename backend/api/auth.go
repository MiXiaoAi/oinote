...
func (h *AuthHandler) GetStats(c *fiber.Ctx) error {
	var stats struct {
		...
		// 最近创建的笔记
		RecentNotes []models.Note `json:"recent_notes"`
		// 今日新增
		TodayNewUsers  int64 `json:"today_new_users"`
		TodayNewNotes  int64 `json:"today_new_notes"`
		TodayNewMessages int64 `json:"today_new_messages"`
		// 频道笔记 vs 个人笔记
		ChannelNoteCount int64 `json:"channel_note_count"`
		PersonalNoteCount int64 `json:"personal_note_count"`
	}
	...
	h.DB.Model(&models.ChannelMessage{}).Count(&stats.MessageCount)
	h.DB.Model(&models.Attachment{}).Count(&stats.AttachmentCount)
	h.DB.Model(&models.Attachment{}).Select("COALESCE(SUM(file_size), 0)").Scan(&stats.TotalFileSize)

	// 最近注册用户（最多5个）
	h.DB.Order("created_at DESC").Limit(5).Find(&stats.RecentUsers)
	for i := range stats.RecentUsers {
		stats.RecentUsers[i].Password = ""
	}

	// 最近创建的笔记（最多5个，预加载所有者信息）
	h.DB.Order("created_at DESC").Limit(5).Preload("Owner").Find(&stats.RecentNotes)

	// 今日新增统计
	today := time.Now().Format("2006-01-02")
	h.DB.Model(&models.User{}).Where("DATE(created_at) = ?", today).Count(&stats.TodayNewUsers)
	h.DB.Model(&models.Note{}).Where("DATE(created_at) = ?", today).Count(&stats.TodayNewNotes)
	h.DB.Model(&models.ChannelMessage{}).Where("DATE(created_at) = ?", today).Count(&stats.TodayNewMessages)

	// 频道笔记 vs 个人笔记
	h.DB.Model(&models.Note{}).Where("channel_id IS NOT NULL").Count(&stats.ChannelNoteCount)
	h.DB.Model(&models.Note{}).Where("channel_id IS NULL").Count(&stats.PersonalNoteCount)
